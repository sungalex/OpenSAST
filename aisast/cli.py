...
예시:
  $ aisast scan ./my-project --no-second-pass --no-triage
  $ aisast serve --host 0.0.0.0 --port 8000
  $ aisast init-db
  $ aisast list-mois
  $ aisast engines
"""
...
from aisast.db.session import init_engine
from aisast.engines.registry import available_engines
from aisast.mois.catalog import MOIS_ITEMS, ensure_49_items
from aisast.orchestrator.pipeline import ScanOptions, run_scan
from aisast.reports.sarif import build_sarif as build_sarif_from_rows
from aisast.sarif.normalize import findings_to_sarif
from aisast.utils.paths import safe_write_text
...
def serve(
    ...
    uvicorn.run("aisast.api.app:app", host=host, port=port, reload=reload)
...
def report(
    ...
    from aisast.db import models as dbm
