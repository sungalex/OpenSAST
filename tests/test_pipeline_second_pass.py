...
from opensast.orchestrator.pipeline import ScanOptions, ScanPipeline


class TestSecondPassCondition:
    """사용자가 엔진을 지정해도 2nd pass 엔진이 포함되면 실행해야 함."""

    @patch("opensast.orchestrator.pipeline.build_engine")
    @patch("opensast.orchestrator.pipeline.emit_hook", return_value=[])
