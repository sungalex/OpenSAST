...
> **v0.5.0**: Dockerfile이 multi-stage 빌드로 전환되었고, non-root 사용자(`aisast`)
...
| `OPENSAST_WORK_DIR` | `<repo>/.aisast-work` | 임시 작업 디렉터리 |

### 3.2 데이터베이스 / 큐

| 변수 | 기본값 |
|------|--------|
| `OPENSAST_DATABASE_URL` | `postgresql+psycopg2://aisast:aisast@localhost:5432/aisast` |
...
| `OPENSAST_MINIO_BUCKET` | `aisast-sources` |
...
| `OPENSAST_BOOTSTRAP_ADMIN_EMAIL` | `admin@aisast.local` |
| `OPENSAST_BOOTSTRAP_ADMIN_PASSWORD` | `aisast-admin` |
| `OPENSAST_BOOTSTRAP_ADMIN_DISPLAY_NAME` | `aiSAST Admin` |
...
- **중복 제거**: `aisast.sarif.merge.merge_findings()`는 `finding_id`(해시) 및 `(파일, 라인, CWE)` 조합을 키로 사용하며, `_ENGINE_PRIORITY`에 따라 우선 엔진을 남긴다. 동일 위치에서 중복 시 severity가 더 높은 쪽을 유지한다(v0.5.0에서 LOW가 MEDIUM을 이기던 비교 버그 수정).
...
- **CSRF 미들웨어**: cloud 프로파일에서 자동 활성화. 쿠키(`aisast_csrf`) + 헤더(`X-CSRF-Token`) 이중 검증. `/api/auth/login`, `/api/auth/refresh`, `/health`, `/ready`, `/metrics` 는 면제.
...
- **Dockerfile 하드닝**: multi-stage 빌드, non-root user(`aisast`), HEALTHCHECK
  내장. 이미지 크기 감소 및 컨테이너 보안 강화.
- **Prometheus `/metrics`**: HTTP 요청 수·지연시간, 스캔 수, 탐지 건수 메트릭
  노출. `aisast/api/middleware/prometheus.py`.
...
- **신규 백엔드 테스트 71건** (총 102 passing):
  ...
  - persist_scan_result 가 suppression 매칭 시 status='excluded' 자동 처리하는지
    실제 DB 시뮬레이션으로 검증
- **자동 마이그레이션 SQLite 호환성 수정**: `aisast/db/migrate.py` 의 ALTER TABLE
...
- **NL Search 박스** (`src/components/NlSearchBox.tsx`): 인디고 그라데이션
  배경, "Sparrow 에 없는 차별화 기능" 표시, 결과 미리보기.
...
### 2026-04-15 (저녁 — Sparrow 대비 엔터프라이즈 고도화 v0.2.0)
스패로우 SAST/SAQT 사용설명서(202쪽) 를 분석한 뒤 격차를 메우는 대규모 기능
릴리스. 새 라우터 5개, 새 DB 테이블 5개, 새 스키마 12개, 새 테스트 9개.

- **다중 레퍼런스 매핑**: `aisast/mois/references.py` — CWE Top 25(SANS 2023),
...
- **이슈 상태 워크플로**: Finding 에 `status / status_reason / reviewed_by /
  ...
  자동 감사 로그 기록. Sparrow 의 '이슈 제외 신청/승인' 워크플로 대응.
...
- **자연어 이슈 검색 (aiSAST 차별화)**: `/api/findings/ask` — 한국어 질의를
  LLM 이 필터 JSON 으로 변환해 검색. LLM 부재 시 키워드 fallback. Sparrow 에
  없는 기능.
...
- **체커 그룹(RuleSet)**: `rule_sets` 테이블 + `/api/rule-sets` CRUD. 프로젝트는
  ...
  최소 심각도, 단일 default 강제. Sparrow '체커 그룹' 대응.
...
- **이전 분석 비교 (diff)**: `/api/scans/{id}/diff?base={prev}` — `finding_hash`
  ...
  카운트 별도 계산. Sparrow '이전 결과 비교' 탭 대응.
- **CI/CD 빌드 게이트(이관 제어)**: `gate_policies` 테이블 + `/api/gate/policy`
  ...
  필드를 보고 머지 차단. Sparrow '이관 제어' 대응.
- **소스 파일 뷰어**: `/api/scans/{id}/source?path=…` — 스캔 작업 디렉터리 내
  파일 내용을 반환(경로 탈출 차단, 512KB 상한, 큰 파일은 truncated). Sparrow
...
- **신규 테스트**: `test_references.py` (5건), `test_finding_workflow.py` (4건)
  → 총 31 passing.
- **신규 §18, §19 추가** (Sparrow 비교 매트릭스, 엔터프라이즈 기능 가이드).
- **자동 컬럼 마이그레이션**: `aisast/db/migrate.py::auto_migrate()` — 모델과 실제 DB 컬럼을 SQLAlchemy `inspector` 로 비교해 누락 컬럼을 `ALTER TABLE … ADD COLUMN IF NOT EXISTS` 로 자동 추가. NULLABLE 또는 default 가 있는 경우만 안전 처리. API startup 이벤트가 호출. 향후 모델 변경에도 컨테이너 재빌드만으로 스키마가 따라간다 (Alembic 도입 전 임시 메커니즘).
...
- **공유 볼륨**: `aisast-work` named volume 신설. api·worker 가 `/var/aisast-work` 로 동일 마운트. `OPENSAST_WORK_DIR=/var/aisast-work` 환경변수 주입.
...
- **Bootstrap**: API `startup` 이벤트에서 `ensure_bootstrap_admin()` 자동 실행. 기본 계정 `admin@aisast.local / aisast-admin`. env: `OPENSAST_BOOTSTRAP_ADMIN_EMAIL/PASSWORD/DISPLAY_NAME`.
...
## 18. Sparrow SAST 대비 기능 비교

스패로우 SAST/SAQT 사용설명서(202쪽) 분석 후 작성한 비교 매트릭스. ✓ = 완전
지원, ◐ = 부분 지원, ✗ = 미지원. ★ 표시는 **aiSAST 가 Sparrow 대비 우위**.

### 18.1 분석·룰

| 기능 | Sparrow | aiSAST | 비고 |
|------|---------|--------|------|
| 다중 엔진 통합 | ✓ (자체) | ✓ (Opengrep, Bandit, ESLint, gosec, SpotBugs+FSB, CodeQL) | aiSAST 는 6개 OSS 엔진 오케스트레이션 |
| 행안부 보안약점 카탈로그 | ✓ (2019) | ✓ (**2021 최신**) ★ | 49개 항목, 7 카테고리 |
| CWE 매핑 | ✓ | ✓ | 카탈로그 단일 소스 |
| OWASP Top 10 매핑 | ✓ | ✓ (2021) | `references_for_cwe()` |
| SANS/CWE Top 25 | △ | ✓ (2023) ★ | aiSAST 자동 매핑 |
| PCI DSS 매핑 | ✗ | ✓ ★ | 핵심 요구사항 |
...
| VS Code 플러그인 | ✓ | ✗ (로드맵) |
| CLI 도구 | ✓ (자체) | ✓ (`aisast scan/serve/...`) |
| REST API | ✓ | ✓ (FastAPI + OpenAPI `/docs`) |
| 인증 | ✓ | ✓ (JWT + bcrypt) |

### 18.6 차별화 한 줄 요약

aiSAST 는 Sparrow 의 모든 핵심 워크플로(상태 관리, 제외 승인, 빌드 게이트,
대시보드, diff, 체커 그룹, 감사 로그) 를 동등 이상으로 제공하면서, **Sparrow에
...
```bash
OPENSAST_MOIS_CATALOG_PATH=/etc/aisast/mois_override.yaml
```
...
```bash
OPENSAST_REFERENCE_STANDARDS_PATH=/etc/aisast/refs_override.yaml
```
...
OPENSAST_CUSTOM_RULES_DIR=/etc/aisast/my-rules \
  docker compose up -d
```
