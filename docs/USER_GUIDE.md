...
| `AISAST_LLM_DEFAULT_FP_PROBABILITY` | `50` | LLM 호출 실패/파싱 오류 시 기본 오탐 확률(0-100) |

### 3.6 관측성

| 변수 | 기본값 | 설명 |
|------|--------|------|
| `AISAST_OTEL_ENABLED` | `false` | OpenTelemetry 트레이싱 활성화 (`true`/`1`/`yes`) |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | *(없음)* | OTLP 수집기 주소 (예: `http://jaeger:4317`) |
| `AISAST_LOG_FORMAT` | `console` | 로그 형식. `json`으로 설정 시 구조화 JSON 로깅 |
...
4. **결과 파싱**: 응답에서 첫 JSON 객체를 추출하여 `TriageResult`로 변환. 파싱 실패 시 `verdict=needs_review`, `fp_probability=AISAST_LLM_DEFAULT_FP_PROBABILITY`(기본 50).
...
- **OpenTelemetry 트레이싱**: `AISAST_OTEL_ENABLED=true` +
  `OTEL_EXPORTER_OTLP_ENDPOINT` 설정으로 분산 트레이싱 활성화.
- **JSON 구조화 로깅**: `AISAST_LOG_FORMAT=json` 설정 시 ELK/Loki 연동용 JSON
  로그 출력.
...
| `cloud` | 프로덕션 클라우드 | **docs 비활성**, **HSTS 강제**, secret 강도 검증, rate 60/min, JSON 로그(`AISAST_LOG_FORMAT=json`), CORS allowlist 강제, **CSRF 활성**, **CSP nonce** |
