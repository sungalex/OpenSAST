...
openSAST는 2-Pass 파이프라인 · 6개 엔진 통합 · MOIS 49 카탈로그 · LLM Triage 원본 보존 원칙 · 플러그인 레지스트리 · 3-프로파일 설정 등 **구조적 완성도는 매우 높다**. 반면, 실제 사용 가치를 결정짓는 **(1) YAML 룰 커버리지**, **(2) 엔터프라이즈 운영 기반**(멀티테넌시/관측성/시크릿/컨테이너 보안), **(3) 몇몇 파이프라인 결함**(severity 비교 버그, 2nd pass 자동 비활성 조건, 하드코딩 상수)이 남아있다.
...
| 영역 | 위치 | 평가 |
|---|---|---|
| 2-Pass 파이프라인 | `opensast/orchestrator/pipeline.py` | `FIRST_PASS_ENGINES` / `SECOND_PASS_ENGINES` 분리 구조 명확, LLM triage 통합까지 단일 진입점 |
| MOIS 49 카탈로그 | `opensast/mois/catalog.py` | 49개 항목 전부 등록, CI에서 `assert len(MOIS_ITEMS) == 49` 강제 |
| LLM Triage 원칙 | `opensast/llm/triage.py:31-34` | **원본 Finding 절대 제거 금지** 원칙 주석+코드로 강제, Ollama/Anthropic/Noop 3개 프로바이더 |
| 플러그인 레지스트리 | `opensast/plugins/registry.py` | `Registry[T]` 제네릭, entry_points 자동 발견, `OPENSAST_PLUGINS_DISABLED` 화이트리스트 |
| 3-프로파일 설정 | `opensast/config.py` | `local/docker/cloud` 자동 기본값 + `validate_profile()` 배포 전 경고 |
| RBAC + 계정 잠금 + 비번정책 | `opensast/api/security.py` | bcrypt NFC 정규화, 4종 문자 클래스 검증, 공통 암호 블랙리스트, 계정 잠금 |
| 리포트 4종 생성기 | `opensast/reports/` | SARIF / HTML / PDF / Excel 모두 구현, 한글 폰트 지원 |
...
| # | 위치 | Gap | 심각도 |
|---|---|---|---|
| O1 | `opensast/orchestrator/pipeline.py:66` | `enable_second_pass and not options.engines` — 사용자가 엔진 목록을 지정하면 2nd pass 가 **조용히 전체 비활성**됨. 직관에 반함 | 🟠 |
| O2 | `opensast/orchestrator/pipeline.py:97-126` | `_run_pass` 루프에 **진행률 콜백/훅 없음** → 프론트 실시간 표시 불가 | 🟠 |
| O3 | `opensast/orchestrator/pipeline.py:74-78` | Triage 실패 시 `needs_review` 자동 설정은 하지만 실패 원인 구조화 로깅 부족 | 🟡 |
| O4 | `opensast/orchestrator/tasks.py:25-57` | `@celery_app.task()` 에 `retry` / `max_retries` / `acks_late` / `autoretry_for` 전혀 없음 | 🟠 |
| O5 | `opensast/orchestrator/tasks.py` | Celery `revoke/terminate` 경로 없음 → **스캔 취소 불가** | 🟠 |
| O6 | `opensast/orchestrator/celery_app.py:61` | `task_time_limit=3600` 하드코딩 — 대형 프로젝트 CodeQL 빌드 시 초과 가능 | 🟡 |
| O7 | `opensast/orchestrator/tasks.py:60-112` | `clone_and_scan_task` 에서 임시 디렉터리 삭제 타이밍이 스캔 완료 이후 — 대용량 저장소 시 디스크 폭주 위험 | 🟡 |
| O8 | `opensast/engines/base.py:50-54` | `BinaryNotFound` 만 처리, `TimeoutExpired` / `ResourceError` / `DiskFull` 미처리 | 🟠 |
| O9 | `opensast/engines/codeql.py:64-72` | `database create` 실패 시 stderr 로그 없음, analyze 전 DB 상태 검증 없음 | 🟠 |
...
| # | 위치 | Gap | 심각도 |
|---|---|---|---|
| D1 | `opensast/db/models.py` 전반 | **복합 인덱스 0개**. Finding(scan_id, severity, status), Scan(project_id, status, created_at), SuppressionRule(project_id, kind), AuditLog(user_id, created_at) 등 핵심 쿼리 풀스캔 | 🟠 |
| D2 | `opensast/db/models.py` | `organization_id` FK 없음 → **싱글테넌시**, 조직 격리 불가 | 🟠 |
| D3 | `opensast/db/models.py:100-107` | Finding.status 전이 규칙이 **주석**으로만 존재, DB 제약(enum) 없음 | 🟡 |
| D4 | `opensast/db/migrate.py:31-56` | `auto_migrate` 가 컬럼 **추가만** 지원 (drop/rename 불가), `alembic/versions/` 에 초기 마이그레이션 1개 | 🟠 |
| D5 | `opensast/services/base.py:65-85` | `session.commit()` 을 서비스에서 직접 호출 — 라우트 응답 후 예외 시 부분 커밋 위험. 트랜잭션 경계 불명확 | 🟡 |
| D6 | `opensast/services/finding_service.py:82-88`, `gate_service.py:115-147` | `path_glob` / text LIKE 필터가 DB 아닌 메모리에서 처리, N+1 위험 | 🟠 |
| D7 | `opensast/services/finding_service.py:46-100` | `search()` 가 offset-based 페이지네이션만 — 대규모 결과셋 느림 | 🟡 |

### 3.5 API / 인증 / 미들웨어

| # | 위치 | Gap | 심각도 |
|---|---|---|---|
| A1 | `opensast/api/security.py:36-42` | JWT payload 에 `iat` / `jti` / `aud` / `iss` 없음 | 🟠 |
| A2 | `opensast/api/security.py` | **Refresh token 없음**, 블랙리스트 없음 — 로그아웃 즉시 무효화 불가 | 🟠 |
| A3 | `opensast/api/deps.py:26-38` | 매 요청마다 DB 조회로 user 확인 — 권한 변경 즉시 반영되지만 성능 저하. Redis 캐시 필요 | 🟡 |
| A4 | `opensast/api/routes/auth.py:44,81` | 로그인 실패 시 타이밍 공격 완화 없음 (응답 시간 차이로 계정 존재 유추 가능) | 🟡 |
| A5 | `opensast/api/middleware/rate_limit.py` | slowapi **in-memory** → 다중 인스턴스 배포 시 우회 | 🟠 |
| A6 | `opensast/api/middleware/security_headers.py:36-46` | CSP `unsafe-inline` 허용 → XSS 위험, nonce/hash 미적용 | 🟠 |
| A7 | `opensast/api/` 전체 | **CSRF 토큰 전무** — SPA POST/PUT 취약 | 🟠 |
| A8 | `opensast/api/middleware/__init__.py:29-36` | CORS `allow_methods="*"`, `allow_headers="*"` 와일드카드 | 🟡 |
| A9 | `opensast/config.py:122-124` | bootstrap admin 자격증명 env 평문, 시크릿 매니저(Vault/AWS SM) 미연동 | 🟠 |
| A10 | `opensast/api/routes/findings.py:113-149` | `/ask` (NL 검색) LLM 에러 로깅 미흡, fallback 동작 불투명 | 🟡 |
...
| C9 | `opensast/utils/logging.py` | JSON 구조화 로깅 옵션 없음 (Rich 콘솔 전용) | 🟡 |
| C10 | `opensast/api/app.py:67-83` | `/health`/`/ready` 가 정적 응답 — DB/Redis/Celery broker ping 미포함 | 🟡 |
...
| F12 | `opensast/cli.py` | 로컬 스캔이 `run_scan` 직접 호출 — Celery 우회, 대용량 시 메모리 오버헤드 | 🟡 |

### 3.9 리포트 / MOIS 카탈로그

| # | 위치 | Gap | 심각도 |
|---|---|---|---|
| P1 | `opensast/reports/pdf.py:19-22` | WeasyPrint import 실패 시 HTML 원본 반환 — PDF 생성 실패 **은폐** | 🟡 |
| P2 | `opensast/reports/pdf.py` | MOIS 공식 보고서 **CSS 템플릿 없음** (표지/목차/요약/상세/조치권고) | 🟠 |
| P3 | `opensast/reports/excel.py:94-105` | 3시트 구성은 되어 있으나 openpyxl **차트 미사용**, pivot 없음 | 🟢 |
| P4 | `opensast/mois/references.py:329-380` | 내장 CWE→OWASP/SANS/PCI 매핑은 하드코딩, 사용자 오버레이는 로드만 되고 통합 미흡 | 🟡 |
| P5 | `opensast/mois/loader.py:36-60` | YAML 오버레이 병합 시 dict 순서 의존 — 결과 재현성 불안정 | 🟡 |
| P6 | `opensast/plugins/registry.py` | `report_registry` 정의되나 `opensast/reports/__init__.py` 는 **하드코드된 `build_sarif`/`build_html`** 직접 호출 — 플러그인 확장 경로 미활용 | 🟡 |
...
4. **`Dockerfile` 하드닝**
   ...
   - `RUN useradd -m -u 10001 opensast && USER opensast`
   - `HEALTHCHECK --interval=30s --timeout=5s CMD curl -f http://localhost:8000/health || exit 1`
5. **CI 커버리지 + 품질 게이트**
   - `pytest -q --cov=opensast --cov-report=xml --cov-report=term-missing`
   - Codecov 업로드 (`codecov/codecov-action@v4`)
   - `mypy opensast` (type check) — 실패 허용 단계 포함해서 점진 도입
...
- `opensast/utils/logging.py` 에 JSON 포맷 옵션 추가 (cloud 기본)
...
- **GitHub Action** (`opensast-action@v1`), **GitLab CI 템플릿**, **Jenkins 플러그인** — 최소 2개 실배포
...
| PR# | 제목 | 예상 소요 | 대상 파일 |
|---|---|---|---|
| 1 | `fix(merge): prevent LOW beating MEDIUM in severity comparison` | 30분 | `opensast/sarif/merge.py`, `tests/test_sarif_merge.py` (신규) |
| 2 | `fix(pipeline): allow second pass when custom engines include codeql/spotbugs` | 1시간 | `opensast/orchestrator/pipeline.py`, `tests/test_pipeline.py` |
...
| 5 | `feat(db): add composite indexes for findings/scans/audit` | 3시간 | `alembic/versions/20260417_0002_composite_indexes.py` (신규), `opensast/db/models.py` |
...
| 8 | `refactor(llm): move hardcoded fp_probability=50 to settings` | 30분 | `opensast/config.py`, `opensast/llm/triage.py` |
...
| 6 | **엔진 바이너리 배포 복잡성** — 사용자가 직접 설치 필요 | 온보딩 마찰 | v0.5~v0.6 중 CodeQL/SpotBugs 포함 "fat" Docker 이미지 빌드 자동화. 엔진 번들 별도 태그(`opensast:v0.5.0-full`) |
| 7 | **프론트 리팩터 vs 기능 개발 트레이드오프** — v0.7 에서 React Query 도입은 파괴적 | 리그레션 | 페이지 단위 점진 이관 — 새로 만드는 페이지부터 React Query 사용, 기존 페이지는 유지 |
| 8 | **데이터 정합성 — `auto_migrate` 프로덕션 사용 금지** | `opensast/db/migrate.py` 가 drop/rename 불가인데 프로덕션에 사용되면 스키마 drift | v0.5 에서 `auto_migrate` 비활성화 (로컬 전용), 프로덕션은 Alembic `upgrade head` 강제. USER_GUIDE 에 명시 |
...
```python
# Severity 는 str Enum (opensast/mois/catalog.py:14-19)
...
```python
# opensast/orchestrator/pipeline.py:61-69
...
`opensast/db/models.py` 전체에서 `__table_args__` 선언 0건. 유일한 제약은:
- `User.email` (unique)
- `Project.name` (unique)
- `RuleSet.name` (unique)
