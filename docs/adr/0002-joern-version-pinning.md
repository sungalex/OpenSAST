...
- 상태(Status): **Proposed — 전제 노후화로 보류** (2026-08-26 확인)

> ⚠️ **이 ADR 은 현재 상태로는 Accepted 가 될 수 없다.**
>
> 본문은 Joern **`v2.0.x` 라인**에서 patch 를 선정하는 것을 전제로 쓰였다
> (2026-04-24 작성). 그러나 2026-08-25 기준 Joern 최신 릴리스는 **v4.0.611** 로
> 두 메이저 앞서 있고, `v2.0.x` 는 유지보수 대상이 아니다. §3 의 선정 기준
> (CVE·CPG 빌드 성공률·골든 일치율)은 그대로 유효하지만 **적용 대상 라인이
> 틀렸다.**
>
> Accepted 로 가려면 **rev.2 재작성**이 필요하다:
> 1. 대상 라인을 `4.0.x` 로 교체하고 §4.1 「왜 v2.0.x 라인인가」를 다시 쓴다
> 2. §2 의 CLI·DSL breaking change 이력을 4.0 기준으로 재조사한다
>    (`joern-scan --overlays`, `cpg.call.methodFullName` 등의 현행 여부)
> 3. `scripts/joern/select_version.py` 와 골든 픽스처를 실제로 만든다 — 지금은 없다
> 4. 선정 결과를 `.env.versions` 에 커밋한다 — 이 파일도 아직 없다
>
> 상위 결정인 [ADR-0001](0001-unified-analysis-pipeline.md) 은 2026-08-26 에
> **Accepted** 가 되었다. 방향은 확정됐고, 막혀 있는 것은 이 버전 고정 절차다.
- 작성일: 2026-04-24
...
- (2026-08-26) 상태 점검: 본문 미수정. 전제한 `v2.0.x` 라인이 현행 `4.0.x` 와
  어긋나 Proposed 유지. rev.2 재작성이 선행돼야 한다.
